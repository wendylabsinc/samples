...
Serves a single web app on :8080
...
def start_web_server():
    """Start Flask in a daemon thread so readiness on :8080 passes immediately."""
    app = build_web_app()
    t = threading.Thread(
...
def main():
    os.makedirs(EVENTS_DIR, exist_ok=True)
    # Web server first so the readiness probe on :8080 passes before the slow
